...
function normalizeDateKey(value) {
  ...
  const plMatch = textValue.match(/(\d{2})[./-](\d{2})[./-](\d{4})/);
  if (plMatch) return `${plMatch[3]}-${plMatch[2]}-${plMatch[1]}`;
  return textValue.slice(0, 10);
}

function reservationMatchesDay(item, dateValue) {
  const pickupDate = normalizeDateKey(item.pickupBy);
  if (pickupDate) return pickupDate === dateValue;
  const reservedDate = normalizeDateKey(item.reservedAt || item.createdAt);
  return reservedDate === dateValue;
}
...
function buildDailyReservationReport(db, dateValue) {
  ...
  for (const reservation of reservations) {
    ...
    reservationBlocks.push({
      customerName: asciiPdfText(reservation.customerName || "-"),
      customerContact: asciiPdfText(reservation.customerContact || "-"),
      pickupBy: asciiPdfText(reservation.pickupBy || "-"),
      inventoryLines,
      tankLines,
      notes: asciiPdfText(reservation.notes || "")
    });
  }
  ...
  const lines = [
    ...
    "Uklad: klient / magazyn / tank / notatki",
    ""
  ];

  if (reservationBlocks.length === 0) {
    lines.push("Brak rezerwacji dla wybranego dnia.");
  } else {
    for (let index = 0; index < reservationBlocks.length; index += 1) {
      const block = reservationBlocks[index];
      lines.push(`Zamowienie ${index + 1}`);
      lines.push(`1. Klient: ${block.customerName}`);
      lines.push(`   Kontakt: ${block.customerContact}`);
      lines.push(`   Odbior: ${block.pickupBy}`);
      lines.push("2. Rzeczy z magazynu:");
      if (block.inventoryLines.length === 0) {
        lines.push("   - Brak");
      } else {
        for (const line of block.inventoryLines) lines.push(`   ${line}`);
      }
      lines.push("3. Rzeczy z tankow, ktore trzeba polac:");
      if (block.tankLines.length === 0) {
        lines.push("   - Brak");
      } else {
        for (const line of block.tankLines) lines.push(`   ${line}`);
      }
      lines.push("4. Notatki do zamowienia:");
      lines.push(block.notes ? `   ${block.notes}` : "   - Brak");
      lines.push("");
    }
  }
